...
const EditTask = () => {
    const { setAddTaskStatus, setEditTaskStatus, setViewTaskStatus, goToToday, updateTask, selectedDate, selectedItem } = useContext(appContext);
    
    const [alarm, setAlarm] = useState(true)
    const [title, setTitle] = useState(selectedItem.id != 999999999 && selectedItem?.title || "")
    const [time, setTime] = useState("11:00")
    // const [editTitle, setEditTitle] = useState("11:00")

    const CloseEditTaskActivate = () => {
  ...
  return (
    <div className='flex flex-col w-11/12 rounded-lg py-4 shadow-xl'>
        <div className='flex flex-row justify-between mx-4'>
            <h3>Edit Task</h3>
            <span onClick={CloseEditTaskActivate} className='text-[1.5rem] rotate-45 cursor-pointer'>+</span>
        </div>
        <div className='mt-2 mx-4 overflow-hidden'>
            <textarea value={title} onChange={(e) => setTitle(e.target.value)} name="" placeholder='Create a video ad' id="" className='w-full h-[8rem] rounded-lg bg-gray-200 border border-gray-400 resize-none outline-none p-2 no-scroll-bar' ></textarea>
        </div>
        <div className='mx-4 mt-4 flex flex-col gap-2'>
            <div className='flex flex-row justify-between'>
                ...
                <div className='flex gap-1 border border-gray-400 py-1 rounded-lg px-2 cursor-pointer bg-white'>
                    ...
                    <select value={selectedItem.id != 999999999 ? selectedItem?.time: ""} onChange={(e) => setTime(e.target.value)} name="" id="" defaultValue={"11:00"} className='native-appearance-none outline-none no-scroll-bar cursor-pointer'>
